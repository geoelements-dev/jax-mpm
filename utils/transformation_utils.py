...
def transform2origin(position_tensor):
    ...
    scale = 1.0 / max_diff
    original_mean_pos = original_mean_pos.to(device=device)
...
def get_mat_from_upper(upper_mat):
    ...
    mat = torch.zeros((upper_mat.shape[0], 9), device=device).double()
    mat[:, :3] = upper_mat[:, :3]
    mat[:, 3] = upper_mat[:, 1]
    mat[:, 4] = upper_mat[:, 3]
    mat[:, 5] = upper_mat[:, 4]
...
def get_uppder_from_mat(mat):
    mat = mat.view(-1, 9)
    upper_mat = torch.zeros((mat.shape[0], 6), device=device).double()
    upper_mat[:, :3] = mat[:, :3]
    upper_mat[:, 3] = mat[:, 4]
    upper_mat[:, 4] = mat[:, 5]
    upper_mat[:, 5] = mat[:, 8]
...
def shift2center111(position_tensor):
    tensor111 = torch.tensor([1.0, 1.0, 1.0], device=device).double()
...
def apply_inverse_rotation(position_tensor, rotation_matrix):
    rotated = torch.mm(position_tensor, rotation_matrix).double()
    return rotated
